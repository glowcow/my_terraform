...
typeset deploy_prod_db_host=192.168.0.100
...
#	postgresql-server postgresql postgresql-contrib 
    ...
    sudo yum -y install \
     postgresql-devel \
	nginx nodejs yarn \
	libxslt-devel libxml2-devel glibc-devel gcc-c++ libxml2-devel shared-mime-info
  ...
  #   fi
    ...
    sudo gpg2 --keyserver hkp://sks.pod02.fleetstreetops.com --recv-keys 409B6B1796C275462A1703113804BB82D39DC0E3 7D2BAF1CF37B13E2069D6956105BD0E739499BDB
    fi
...
ExecStartPre=/home/username/2_wait_pg_server_and_migrate.sh
...
checks() {
    ...
    if (( ! UID )); then
	echo_err "Please run this script as non-root user (with sudo permissions)" >&2
    fi
