output "arn_id" {
  ...
  value       = join(", ", aws_instance.linux-aws.*.arn)
}

output "image" {
  description = "images of EC2"
  value       = join(", ", aws_instance.linux-aws.*.ami)
}

output "ipv4_address" {
  description = "public IPv4 addresses assigned to the EC2"
  value       = join(", ", aws_instance.linux-aws.*.public_ip)
}

output "pub_dns" {
  description = "public dns assigned to the EC2"
  value       = join(", ", aws_instance.linux-aws.*.public_dns)
}

output "ipv4_address_private" {
  description = "private IPv4 addresses assigned to the EC2, if applicable"
  value       = join(", ", aws_instance.linux-aws.*.private_ip)
}
